...
<form method="post" name="affiliateListBasic" action="#Variables.formAction#">
...
<input type="hidden" name="searchField" value="affiliateName,affiliateCode,affiliateURL,affiliateID_custom">
<td>
	...
	Search Text: <input type="text" name="searchText" value="#HTMLEditFormat(Form.searchText)#" size="25" class="TableText"> <input type="submit" name="submitAffiliateList" value="Submit">
</td>
...
<tr bgcolor="E7D8F3" valign="top">
	<td>
		...
		<tr>
			<td>
				<label><input type="checkbox" name="searchField" value="affiliateName"<cfif ListFind(Form.searchField, "affiliateName")> checked</cfif>>Name</label><br>
				<label><input type="checkbox" name="searchField" value="affiliateCode"<cfif ListFind(Form.searchField, "affiliateCode")> checked</cfif>>Code</label><br>
			</td>
			<td>
				<label><input type="checkbox" name="searchField" value="affiliateURL"<cfif ListFind(Form.searchField, "affiliateURL")> checked</cfif>>URL</label><br>
				<label><input type="checkbox" name="searchField" value="affiliateID_custom"<cfif ListFind(Form.searchField, "affiliateID_custom")> checked</cfif>>Custom ID</label>
			</td>
		</tr>
		</table>
	</td>
	...
	<td>
		...
		<cfif Application.fn_IsUserAuthorized("exportAffiliates")>
			<img src="#Application.billingUrlRoot#/images/blank.gif" width="5" height="3" alt="" border="0"><br>
			<select name="affiliateIsExported" size="1" class="SearchSelect">
			<option value="-1">-- SELECT EXPORT STATUS --</option>
			<option value=""<cfif Form.affiliateIsExported is ""> selected</cfif>>Not Exported</option>
			<option value="0"<cfif Form.affiliateIsExported is 0> selected</cfif>>Exported - Awaiting Import Confirmation</option>
			<option value="1"<cfif Form.affiliateIsExported is 1> selected</cfif>>Exported - Import Confirmed</option>
			</select><br>
		</cfif>
	</td>

	<cfset Variables.booleanList_value = "affiliateStatus,affiliateHasCode,affiliateHasCustomID,affiliateHasURL,affiliateHasUser,affiliateNameIsCompanyName,affiliateHasCustomer,affiliateHasActiveSubscriber,affiliateHasCustomPricing,affiliateHasCommission">
...
<tr>
	<td colspan="5" bgcolor="ccccff" align="center">
		Display Per Page: <input type="text" name="queryDisplayPerPage" value="#HTMLEditFormat(Form.queryDisplayPerPage)#" size="2" class="TableText"> &nbsp; &nbsp; 
		<input type="submit" name="submitAffiliateList" value="Submit"> &nbsp; &nbsp; <input type="reset" value="Reset">
	</td>
</tr>
</form>
</table>
