...
<cfelse>
	...
	<cfloop Index="pID" List="#Variables.productID_list#">
		...
		<cfloop Index="count" From="1" To="#Form.productSpecCount_orig#">
			<cfif Not ListFind("0,1", Form["productSpecHasImage#count#_#Variables.thisProductID#"])>
				<cfset errorMessage_fields["productSpecHasImage#count#_#Variables.thisProductID#"] = count & "." & Replace(Variables.lang_insertProductSpec.productSpecHasImage, "<<COUNT>>", count, "ALL")>
			</cfif>

			<cfif Len(Form["productSpecValue#count#_#Variables.thisProductID#"]) gt maxlength_ProductSpec.productSpecValue>
				<cfset errorMessage_fields["productSpecValue#count#_#Variables.thisProductID#"] = count & "." & ReplaceNoCase(ReplaceNoCase(ReplaceNoCase(Variables.lang_insertProductSpec.productSpecValue, "<<MAXLENGTH>>", maxlength_ProductSpec.productSpecValue, "ALL"), "<<LEN>>", Len(Form.productSpecValue), "ALL"), "<<COUNT>>", count, "ALL")>
			</cfif>
		</cfloop>
	</cfloop>
</cfif>
